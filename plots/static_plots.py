...
DATA_PATH = Path(r'C:\Users\Mayo\Downloads\FlatIron\website')
...
def load_cluster_waveforms(pid):
    ...
    wf_chns = np.load(DATA_PATH.joinpath(pid, 'clusters.waveformsChannels.npy'))
...
def filter_spikes_by_trial(spikes, tstart, tend):
    idx = np.where(np.bitwise_and(spikes.times >= tstart, spikes.times <= tend))[0]
    return _filter(spikes, idx)
...
def plot_session_raster(spikes, trials, cluster_idx, trial_idx, subsample=100):
    ...
    ax.scatter(spikes.times, spikes.depths, s=spikes.sizes, facecolors='none', edgecolors='r')
...
def plot_trial_raster(spikes, trials, cluster_idx, trial_idx):
    ...
    trials = filter_trials_by_trial_idx(trials, trial_idx)
    spikes = filter_spikes_by_trial(spikes, trials['stimOn_times'] - 2, trials['feedback_times'] + 2)
    alpha = spikes.sizes / np.max(spikes.sizes)
    fig, ax = plt.subplots(1, 1, figsize=(9, 6))
    set_figure_style(fig)
    ax.scatter(spikes.times, spikes.depths, s=spikes.sizes, alpha=alpha, c='grey')
    ax.set_ylim(0, 3840)
...
def add_trial_events_to_raster(ax, trials):
    ...
    for e, c, l in zip(events, colors, labels):
        ...
        ax.text(trials[e], 1.01, l, c=c, rotation=45, rotation_mode='anchor', ha='left', transform=trans)
...
def plot_spikes_amp_vs_depth(clusters, cluster_idx):
    ...
    ax.set_ylim(0, 3840)
    ax.set_xlim(-5, 500)
    ax.scatter(clusters.amps * 1e6, clusters.depths, facecolors='none', edgecolors='grey')
...
def plot_spikes_fr_vs_depth(clusters, cluster_idx):
    ...
    ax.set_ylim(0, 3840)
    ax.set_xlim(-2, 30)
    ax.scatter(clusters.firing_rate, clusters.depths, facecolors='none', edgecolors='grey')
    clusters = filter_clusters_by_cluster_idx(clusters, cluster_idx)
...
def plot_left_right_single_cluster_raster(spikes, trials, cluster_idx):
    ...
    fig, ax = single_cluster_raster(spikes.times, trials['stimOn_times'], trial_idx, dividers, ['g', 'y'], ['left', 'right'])
    set_figure_style(fig)
    ax[0].set_ylim(0, 20)  # TODO this shouldn't be harcoded here
    ax[1].set_yticklabels([])
    set_axis_style(ax[1], xlabel='Time from Stim On (s)', ylabel='Sorted Trial Number')
...
def plot_correct_incorrect_single_cluster_raster(spikes, trials, cluster_idx):
    ...
    ax[0].set_ylim(0, 20)  # TODO this shouldn't be harcoded here
    ax[1].set_yticklabels([])
    set_axis_style(ax[1], xlabel='Time from Feedback (s)', ylabel='Sorted Trial Number')
    set_axis_style(ax[0], ylabel='Firing Rate (Hz)')
...
def single_cluster_raster(spike_times, events, trial_idx, dividers, colors, labels):
    ...
    raster_bin = 0.01
    psth_bin = 0.05
    raster, t_raster = bin_spikes(spike_times, events, pre_time=pre_time, post_time=post_time, bin_size=raster_bin)
    psth, t_psth = bin_spikes(spike_times, events, pre_time=pre_time, post_time=post_time, bin_size=psth_bin)

    dividers = [0] + dividers + [len(trial_idx)]
    fig, ax = plt.subplots(2, 1, figsize=(4, 6), gridspec_kw={'height_ratios': [1, 3], 'hspace': 0}, sharex=True)
    for iD in range(len(dividers) - 1):
        psth_div = np.nanmean(psth[trial_idx[dividers[iD]:dividers[iD + 1]]], axis=0) / psth_bin
        std_div = (np.nanstd(psth[trial_idx[dividers[iD]:dividers[iD + 1]]], axis=0) / psth_bin) \
                  / np.sqrt(dividers[iD + 1] - dividers[iD])

        ax[0].fill_between(t_psth, psth_div - std_div, psth_div + std_div, alpha=0.4, color=colors[iD])
    ...
    secax.set_yticklabels(labels, rotation=90, rotation_mode='anchor', ha='center')
...
def plot_cluster_waveforms(waveforms, waveform_channels, channels, clusters, cluster_idx):
    ...
    wfs, wf_chns = filter_wfs_by_cluster_idx(waveforms, waveform_channels, clusters, cluster_idx)

    fig, ax = plt.subplots(1, 1, figsize=(5, 5))
    set_figure_style(fig)
    wf_chn_pos = np.c_[channels['localCoordinates'][:, 0][wf_chns], channels['localCoordinates'][:, 1][wf_chns]]
...
def on_pid_change(pid, cluster_idx, trial_idx):
    ...
    figs.append(plot_left_right_single_cluster_raster(spikes, trials, cluster_idx))
    figs.append(plot_correct_incorrect_single_cluster_raster(spikes, trials, cluster_idx))
    # fig = plot_contrast_single_cluster_raster() # TODO
    figs.append(plot_cluster_waveforms(waveforms, waveform_channels, channels, clusters, cluster_idx))
